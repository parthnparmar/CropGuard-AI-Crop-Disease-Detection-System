...
// Always start as English on every page load/reload.
// Language only persists within the same tab session, not across reloads.
let currentLang = 'en';
...
function setLanguage(code) {
  ...
  if (code === 'en') {
    // Restore original — click the "Show original" link if present
    const restore = document.querySelector('.goog-te-banner-frame');
    if (restore) restore.contentDocument?.querySelector('#\\:1\\.restore')?.click();
